...
): Promise<{ data: ICourse[]; totalPages: number } | undefined> {
  try {
    connectToDatabase();
    ...
    const [courses, totalItems] = await Promise.all([
      Course.find(query).skip(skip).limit(limit).sort({ created_at: -1 }),
      Course.countDocuments(query), // 👈 đếm tổng số courses matching
    ]);
