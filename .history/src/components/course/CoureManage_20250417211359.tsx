...
const CoureManage = ({ course }: { course: ICourse[] }) => {
  ...
  const handleChangPage = (type: "prev" | "next", page: number) => {
    let newPage;
    if (type === "prev") {
      newPage = Math.max(1, page - 1); // Trang không thể nhỏ hơn 1
    } else {
      newPage = page + 1;
    }

    // Nếu page mới vượt quá số trang hiện tại (dựa trên course.length)
    if (newPage > Math.ceil(course.length / 10)) {
      return; // Nếu trang tiếp theo không hợp lệ, không thay đổi gì
    }
  ...
  const isNextDisabled = course.length <= page * 10; // Thay đổi ở đây
  return (
    <>
      <Link
      ...
      <div className="w-full">
        ...
        <div className="flex justify-center sm:justify-end gap-3 mt-5">
          <button
            ...
            onClick={() => !isPreviousDisabled && handleChangPage("prev", page)} // Chỉ cho phép click khi isPreviousDisabled là false
            disabled={isPreviousDisabled} // Disable button khi trang đầu tiên
          >
            {IconArrowLeft}
          </button>
          <button
            className="size-10 rounded-md flex items-center justify-center bg-primary hover:bg-blue-700"
            onClick={() => !isNextDisabled && handleChangPage("next", page)} // Chỉ cho phép click khi isNextDisabled là false
            disabled={isNextDisabled} // Disable button khi không có trang tiếp theo
          >
            {IconArrowRight}
          </button>
        </div>
