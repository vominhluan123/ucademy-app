...
const OrderMange = ({
  ...
  const handleCancelOrder = ({ orderId }: { orderId: string }) => {
    Swal.fire({
  ...
  const handlerSelectStatus = (status: EOrderStatus) => {
    startTransition(() => {
      updateQueryParams("status", status);
    });
  };
  const handlerSearchOrder = debounce(
          ...
          <div className="flex gap-3">
            <div className="relative w-full sm:w-[300px]">
              <Input
                ...
                onChange={handlerSearchOrder}
              />
            </div>
            <Select
              onValueChange={(value) =>
                handlerSelectStatus(value as EOrderStatus)
              }
            >
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              className={commonClassName.action}
                              onClick={() =>
                                handleCancelOrder({ orderId: orders._id })
                              }
                            >
                              <IconCancel />
                            </Button>
