...
const page = async ({
  ...
  const lessnList = await findAllLessons({ course: courseID || "" });
  if (!lessonDetails) {
    return null;
  }
  const curentLessonIndex = lessnList?.findIndex(
    (el) => el.slug === lessonDetails.slug
  );
  console.log("🚀 ~ curentLessonIndex:", curentLessonIndex);
